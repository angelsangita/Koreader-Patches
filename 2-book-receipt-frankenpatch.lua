...
function quicklookbox:init()
	...
	local db_font_size_big = 20
	local db_font_size_mid = 15
	local db_font_size_small = 13
	local db_padding = 15 -- padding b/w framecontainer and widgets
	local db_padding_internal = 5 -- vertical padding between widgets
	
	function databox(db_typename, db_itemname, db_pagedone, db_pagetotal, show_title) -- one databox each for book and chapter
	...
	if self.ui and self.ui.bookinfo and self.ui.document then
		...
		if cover_bb then
			...
			local max_height = math.floor(Screen:getHeight() / 6) -- Shorter height for header style
			
			-- Scale to maintain aspect ratio, filling the width
			...
			-- Crop from top if taller than max_height
